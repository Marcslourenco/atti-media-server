...
# Remover flag antes do startup para testar que o startup limpa a flag
...
p = subprocess.Popen(['uvicorn', 'main:app', '--host', '127.0.0.1', '--port', '8010'], env=env)
time.sleep(4)

# Criar flag após o startup para simular ingestão concluída
...
try:
    test_cases = [
        {
            "pergunta": "O que é um humano digital?",
            "esperado_nao_contem": ["O que é um humano digital?"]
        },
        {
            "pergunta": "Como você se chama?",
            "esperado_nao_contem": ["Como você se chama?"]
        },
        {
            "pergunta": "O que vocês fazem?",
            "esperado_nao_contem": ["O que vocês fazem?"]
        }
    ]

    for case in test_cases:
        print(f"\nTestando pergunta: '{case['pergunta']}'")
        r = requests.post('http://127.0.0.1:8010/api/avatar/speak', json={
            "text": case["pergunta"],
            "avatar_id": "sofia",
            "language": "pt-BR"
        })
